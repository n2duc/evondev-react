    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm flex flex-col">
            <div className="h-[297px] p-[10px]">
                <img 
                    src={data.poster_path == null ? noImage : `https://image.tmdb.org/t/p/w500${data.poster_path}`}
                    alt="MovieImage"
                    className="w-full h-full object-cover rounded-lg"
                />
            </div>
            <div className="p-[30px] flex-1 flex flex-col">
                <h3 className="text-base text-black font-semibold mb-4">{data.title}</h3>
                <p className="text-[#999999] text-sm mb-6 h-[196px] text-ellipsis overflow-hidden !leading-loose">{data.overview}</p>
                <div className="flex items-center justify-between mt-auto">
                    <div className="flex items-center gap-x-3">
                        ...
                        <span className="text-sm font-semibold text-[#333333]">{data.vote_average}</span>
                    </div>
                    <span className="text-sm font-semibold text-[#333333]">{data.release_date}</span>
                </div>
            </div>
        </div>
    )
